...
def filter_genes(X,genes,thresh=0):
    ...
    keep_inds_MT = np.array([i for i in range(len(genes)) if 'MT-' not in genes[i]])
    keep_inds = np.intersect1d(keep_inds_thresh,keep_inds_MT)
    print 'Kept %d genes for having > %d counts across all cells'%(len(keep_inds),thresh)
...
def low_dimensional_embedding(X,pcs=10):
    xpca = jz_pca(X,showplot=False)
    xtsne = jz_tsne(xpca[:,:pcs],showplot=False)
